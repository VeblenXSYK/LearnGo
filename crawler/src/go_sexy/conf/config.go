...
func (c *Config) Load(file string) error {
	...
	comRegex := regexp.MustCompile(`\s*/\*.*\*/`)    //反引号用来创建原生的字符串字面量
	content = comRegex.ReplaceAll(content, []byte{}) //删除注释（注释前的空格也要删除）

	nRegex := regexp.MustCompile(`\n|\t|\r`)
	content = nRegex.ReplaceAll(content, []byte{}) //删除回车、换行、缩进符
